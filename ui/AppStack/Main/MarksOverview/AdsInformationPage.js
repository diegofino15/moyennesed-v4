...
function AdsInformationPage({ navigation }) {
  ...
  async function getIfVoted() {
    ...
    if (votes.votedForAddPaidAccounts) {
    ...
    if (alreadyVoted) {
      await AsyncStorage.setItem("votes", JSON.stringify({ ...votes, votedForAddPaidAccounts: true }));
      setHasAlreadyVoted(true);
    }
    setIsLoading(false);
  }
      ...
      children={(
        <View>
          ...
          <View style={{ paddingHorizontal: 20, marginTop: 20 }}>
            <Text style={[theme.fonts.labelLarge, { textAlign: "justify", marginBottom: 10 }]}>Les publicités permettent le financement du développement de l'app, pour qu'elle soit mise à jour régulièrement et que les bugs soient réglés au plus vite.</Text>
            <Text style={[theme.fonts.labelLarge, { textAlign: "justify", marginBottom: 10 }]}>Selon vos choix, les publicités sont ou ne sont pas personnalisées.</Text>
              ...
              children={(
                <>
                  ...
                  <View style={{
                    ...
                    {isLoading || confirmingVote ? (
                      <ActivityIndicator size={30} color={theme.colors.onSurfaceDisabled}/>
                    ) : hasAlreadyVoted ? (
                      <Text style={[theme.fonts.labelMedium, { textAlign: "center" }]}>Vous avez déjà voté</Text>
                    ) : hasVoted ? (
